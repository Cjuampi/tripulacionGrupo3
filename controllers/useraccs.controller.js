...
const userAccs = {
    ...
    postLogin: async (req, res) => {
        ...
        try {
            ...
            if (data == undefined) {
                //403
                res.status(200).json({ type: 'Error', message: 'Usuario no registrado, registrate para continuar' })
            } else {
                if (data.hasOwnProperty('numError')) {
                    ...
                    if (data.password == undefined) {
                        ...
                        bcrypt.compare(user.password, data.password, function (err, resp) {
                            if (err) {
                                //500
                                res.status(200).json({ type: 'Error', message: 'No se puede ejecutar la accíon de autenticar, intentelo más tarde' })
                            } else {
                                if (resp) {
                                    let newToken = fncUtils.generateToken(user.email, data.nombre)
                                    ...
                                    try {
                                        if (updateToken == undefined) {
                                            ...
                                            //403
                                            res.status(200).json({ type: 'Error', message: 'No se registra la autenticacion, intentolo más tarde' })
                                        } else {
                                            if (updateToken.hasOwnProperty('numError')) {
                                                /* console.log('numError: ', updateToken) */
                                                //500
                                                res.status(200).json({ type: 'Error', message: 'No se puede escribir la autenticacion, intentelo más tarde' })
                                            } else {
                                        ...
                                        console.log(err)
                                        //500
                                        res.status(200).json({ type: 'Error', message: 'Catch No se puede ejecutar la accíon de autenticar, intentelo de nuevo' })
                                    }
                                } else {
                                    //403
                                    res.status(200).json({ type: 'Error', message: 'Password incorrecta' })
                                }
                            }
                        });
                    }
    ...
    postSignUp: async (req, res) => {
        ...
        const user = {
            ...
            token: fncUtils.generateToken(req.body.userEmail, req.body.userName)
        }
            ...
            if (data == undefined) {
                try {
                    ...
                    if (data.affectedRows == 1) {
                        res.status(200).json({ type: 'Ok', message: 'Usuario creado correctamente', token: user.token, username:fncUtils.decoToken(user.token).username, email:fncUtils.decoToken(user.token).email})
                    } else {//409
